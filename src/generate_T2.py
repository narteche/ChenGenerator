...
class QBC:
    ...
    def convert_to_QDIMACS(self):
        ...
        for G in C.get_gates():
            ...
            x = dic[V[0]] if V[0][0] != '-' else -1 * dic[V[0][1:]]
            y = dic[V[1]] if V[1][0] != '-' else -1 * dic[V[1][1:]]
            z = 0
            
            
            if len(V) == 3:
                z = dic[V[2]] if V[2][0] != '-' else -1 * dic[V[2][1:]]


            if op == "or" and len(V) == 3:
    ...
    def print_formula(self, form='QCIR', output='stdIO', filename=None):
        
        result = ""
        if form=='QCIR':
            result = self.convert_to_QCIR()
        elif form == 'QDIMACS':
            result = self.convert_to_QDIMACS()
        ...
        if output == "stdIO":
            ...
            if filename == None: # default filename
                ext = ".qcir" if form == 'QCIR' else ".qdimacs"
...
def run_generator():
    ...
    if args == None:
        ...
        C = generate_ChenType2(n)
        if form == 'QCIR':
            C.print_formula(form, out, filename)
        else:
            phi = C.convert_to_QDIMACS()
...
def read_arguments():
    ...
    if form == '-QCIR':
        ...
        print("ERROR: Wrong format selection.")
        return None
    ...
    return n, form, out, filename

def print_help():
    print("=== Formula generator for Type 2 Chen Formulas ===")
    print("")
    print("Input should be of the form:")
    print("")
    print("  python3 generate_T2 n -QCIR [filename]  ")
    print("  python3 generate_T2 n -QDIMACS [filename]  ")
    print("")
    print("  - n is a positive integer")
    print("  - -QDIMACS or -QCIR is a the format")
    print("  - and filename is the name of the file (std. output otherwise)")
    print("")
    print("")
    print(" ### Noel Arteche (noel.arteche@gmail.com) ###")
    print("August 2019")
